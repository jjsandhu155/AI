...
def loop(current_table, current_tree, depth):
    ...
    for new_table, value in new_tables:
        if calculate_starting_entropy(new_table) == 0:
            ...
            depth += 1
            print(depth * '  ', value)
            temp_recur = loop(new_table, dict(), depth + 1)
            current_tree[heading[feature]][value] = temp_recur

    return current_tree


def classify(feature_vector, tree):
    ...
    if type(tree) == type(''):
        ...
        for key in tree.keys():
            if key == current:
                classify(feature_vector, tree[current])
    classification = None
    current_tree = tree
    while not classification:
        for key in tree.keys():
            if '?' in key:
                current_tree = tree[key]
